        ...
        @foreach($this->brands->whereNotNull('logo')->where('id', '!=', 1)->where('main', 1) as $brand)
            <div class="col">
                <a class="btn btn-outline-secondary w-100 rounded-4 p-3" href="shop-catalog-electronics.html">
                    <img src="{{ asset('storage/'.$brand->logo) }}" class="d-none-dark"
                         alt="{{ $brand->translations('locale', app()->getLocale())->first()->title ?? ''}}">
                </a>
            </div>
        @endforeach
        <div class="col">
            <a class="btn btn-outline-secondary w-100 h-100 rounded-4 p-3 font-neue"
